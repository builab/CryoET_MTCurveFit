...
def find_seed(
    ...
    while True:
        ...
        if len(candidate_indices) > 0:
            ...
            if len(valid_indices) > 0:
                ...
                if len(seed_indices) >= params['min_seed']:
                    return seed_indices
    ...
    return seed_indices if len(seed_indices) >= params['min_seed'] else []
...
def angle_evaluate(
    ...
    """
    evaluation_step = 40 / params['angpix']
...
def seed_extension(
    ...
    poly_seed_xy = np.poly1d(np.polyfit(ind_vars, dep_vars_xy, params['poly_order_seed']))
    ...
    # 3. Check curvature
    poly_xy_final = np.poly1d(np.polyfit(ind_vars, dep_vars_xy, params['poly_order']))
    ...
    while iteration < max_iterations:
        ...
        poly_xy_growth = np.poly1d(np.polyfit(ind_vars_all, dep_vars_xy_all, params['poly_order']))
        poly_k_growth = np.poly1d(np.polyfit(ind_vars_all, dep_vars_k_all, params['poly_order']))
    ...
    if len(cluster_indices) - len(seed_indices) >= params['min_number_growth']:
        ...
        poly_final_xy = np.poly1d(np.polyfit(ind, dep_xy, params['poly_order']))
        poly_final_k = np.poly1d(np.polyfit(ind, dep_k, params['poly_order']))
...
def process_file(file_path: str, params: Dict[str, Any]) -> None:
    ...
    try:
        coords, tomo_name, detector_pixel_size = load_coordinates(file_path, params['angpix'])
        if coords is None:
